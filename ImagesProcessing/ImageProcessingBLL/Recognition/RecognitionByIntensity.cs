...
namespace ImageProcessingBLL.Recognition
{
    public class RecognitionByIntensity: IRecognizable
    {
        ...
        public int Recognition(List<DistanceByClass> distances)
        {
            ...
            var lst = distances.Where(obj => obj.Distance <= halfOfMaxDistance).GroupBy(obj => obj.ImageClass).Select(obj => new {ImageClass = obj.Key, Count = obj.Count()}).ToList();

            var maxCount = lst.Max(obj => obj.Count);
            return lst.Where(obj => obj.Count == maxCount).First().ImageClass;
